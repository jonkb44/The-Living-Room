...
// A presence row is considered stale (the person has likely left, even if
// their tab never fired a cleanup event) after this many ms without a
// heartbeat update to last_seen_at.
...
export default function RoomPage() {
  ...
  // Once we have a live room + a profile, load history, join presence, and
  // subscribe to realtime updates for messages, reactions, and presence.
  useEffect(() => {
      ...
      .on(
        ...
        async () => {
          ...
          const { data: presenceRows } = await supabase
            .from("room_presence")
            .select("user_id, activity, is_quiet, joined_at, last_seen_at, user_profiles(display_name)")
            .eq("room_id", room.id)
